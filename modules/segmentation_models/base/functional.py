...
def accuracy(gt, pr, class_weights=1, class_indexes=None, smooth=SMOOTH, per_image=False, threshold=None, **kwargs):
    r"""Calculate precision between the ground truth (gt) and the prediction (pr).
    ...
    fp = backend.sum(pr, axis=axes) - tp
    fn = backend.sum(gt, axis=axes) - tp
    tn = backend.sum( (1 - gt) * (1 - pr), axis=axes)
    #tn = backend.sum(backend.sum(~gt, 1) * backend.sum(~pr, 1), axis=axes)

    score = (tp + tn + smooth) /(tp + fp + fn + tn+ smooth)
    score = average(score, per_image, class_weights, **kwargs)
    return score
